...
/**
 * commit阶段的before mutation阶段
 * 1. 处理focus状态
 * 2. 遍历fiber树，执行class组件的getSnapshotBeforeUpdate函数
 * 3. 将根节点下的dom节点移除
 * @param {*} root 
 * @param {*} firstChild 
 * @returns 
...
function commitBeforeMutationEffects_begin() {
  ...
  while (nextEffect !== null) {
    ...
    if (
      ...
      // 如果fiber有update或Snapshot副作用，且存在子fiber，则，将nextEffect指向子fiber，进入下一轮遍历
      child.return = fiber;
      nextEffect = child;
    } else {
...
function commitBeforeMutationEffects_complete() {
  while (nextEffect !== null) {
    ...
    try {
      // 如果fiber有snapshot标识
      // 如果是class组件，则执行getSnapshotBeforeUpdate函数
      // 如果是hostRoot，则将根节点下的dom节点都移除
    ...
    // 如果有兄弟节点，则将nextEffect指向兄弟fiber，并退出循环返回
    ...
    switch (finishedWork.tag) {
      ...
      case ClassComponent: {
        if (current !== null) {
          ...
          // 将返回的快照值，保存在实例的__reactInternalSnapshotBeforeUpdate上，到时候赋值给componentDidUpdate
...
/**
 * 针对fiber进行layout阶段的处理
 *  - 如果是函数组件，则执行useLayoutEffect回调函数
 *  - 如果是类组件，mount阶段则执行componentDidMount函数，update阶段则执行componentDidUpdate函数，最后调用commitUpdateQueue函数
 *  - 如果是原生根组件，则会调用commitUpdateQueue函数
 *  - 如果是原生组件，在mount阶段且存在更新标识，则会调用commitMount函数，处理autoFocus，以及img标签的src赋值
 *  - 最后，处理ref的赋值
 */
    ...
    switch (finishedWork.tag) {
      ...
      case ClassComponent: {
          ...
          if (!offscreenSubtreeWasHidden) {
            if (current === null) {
              ...
              if (
                ...
                // instance.__reactInternalSnapshotBeforeUpdate 存放的是 before-mutation阶段调用getSnaoShotBeforeUpdate方法的返回值
                instance.componentDidUpdate(
                  prevProps,
                  prevState,
                  instance.__reactInternalSnapshotBeforeUpdate,
        ...
        if (updateQueue !== null) {
          ...
          // but instead we rely on them being set during last render.
          // TODO: revisit this when we implement resuming.
          // 处理update回调函数，如this.setState({}, callback)中的callback
...
/**
 * 处理ref，如果ref是函数，则调用ref函数，否则直接将instance赋值给ref.current
 * @param {*} finishedWork 
 */
...
/**
 * 1. 如果有ContentReset标识，则重置文本内容
 * 2. 如果有兄弟节点则使用insertBefore插入DOM节点，否则使用appendChild方法，插入DOM
 * @param {*} finishedWork 
 * @returns 
 */
function commitPlacement(finishedWork: Fiber): void {
  ...
  // 根据父级fiber的类型做不同处理，placement只会在原生节点上标识
  switch (parentFiber.tag) {
    /** ------------- 原生组件 --------------- */
    case HostComponent: {
      const parent: Instance = parentFiber.stateNode;
      if (parentFiber.flags & ContentReset) {
        // 如果父级fiber有ContentReset标识（内容重置）
        // Reset the text content of the parent before doing any insertions
        // 则将parent.contentText = ''
      ...
      // We only have the top Fiber that was inserted but we need to recurse down its
      // children to find all the terminal nodes.
      // 如果有兄弟DOM节点，则使用DOM的insertBefore方法，插入DOM节点，否则使用DOM的appendChild方法
 ...
 * @param {*} deletedFiber 标记删除的fiber节点
 */
...
/**
 * 向下深度遍历子节点，调用commitDeletionEffectsOnFiber，递归执行删除操作
 * @param {*} finishedRoot 
 * @param {*} nearestMountedAncestor 
 * @param {*} parent 
 */
...
/**
 * 对fiber节点执行删除操作
 * 1. 根据节点的不同类型，做不同的处理方式，如函数组件，则执行destroy方法，类组件则执行componentWillUnmount方法
 * 2. 如果是原生组件，则调用DOM节点的removeChild方法移除节点
 * 3. 向下遍历子fiber递归调用此方法，但原生组件的removeChild方法，只会在最上层调用移除
 * @param {*} finishedRoot 根节点FiberRoot
 * @param {*} nearestMountedAncestor 父节点
 * @param {*} deletedFiber 需要删除的fiber节点
 * @returns 
 */
function commitDeletionEffectsOnFiber(
  ...
  switch (deletedFiber.tag) {
    ...
    case SimpleMemoComponent: {
      if (!offscreenSubtreeWasHidden) {
        // 遍历updateQueue，取updateQueue.next，也就是从update链表的第一个update开始遍历
        // 执行destory方法，也就是useInsertionEffect或useLayoutEffect的回调方法的返回函数，useEffect方法不处理
    ...
    case ClassComponent: {
      if (!offscreenSubtreeWasHidden) {
        ...
        // 如果存在componentWillUnmount方法，则执行componentWillUnmount方法
...
function recursivelyTraverseMutationEffects(
  ...
  // 从fiber.deletions从获取需要删除节点的数组，在render阶段beginWork方法中diff过程中收集的需要删除的fiber数组
  const deletions = parentFiber.deletions;
  if (deletions !== null) {
    for (let i = 0; i < deletions.length; i++) {
      const childToDelete = deletions[i];
      try {
        // 对每个删除列表中的fiber节点执行的删除逻辑
  ...
  // 如果fiber上存在子fiber有更新的标识，则进行对子fiber的深度遍历
...
/**
 * 针对fiber节点的DOM操作，根据fiber的不同类型做不同处理
 * 核心：1，深度遍历执行删除操作  2. 执行palcement插入操作
 * 如果是函数组件，依次调用useInsertionEffect的销毁函数和回调函数，以及useLayoutEffect的销毁函数
 * 如果是类组件，清除ref
 * 如果是原生组件，ref清空，判断是否需要内容重置，判断是否需要更新，调用commitUpdate，进行复用DOM节点的属性的更新替换
 * 如果是原生文本组件，进行文本内容的替换
 * 
 * @param {*} finishedWork 
 * @param {*} root 
...
function commitMutationEffectsOnFiber(
  ...
  switch (finishedWork.tag) {
    ...
    case SimpleMemoComponent: {
      ...
      if (flags & Update) {
        // 对于函数组件，会进行useInsertionEffect的销毁函数
        try {
    ...
    case HostComponent: {
      ...
      if (supportsMutation) {
        ...
        // 如果有更新标识，且存在老DOM节点，则调动commitUpdate进行DOM的更新操作
        if (flags & Update) {
          const instance: Instance = finishedWork.stateNode;
          if (instance != null) {
...
/**
 * layout阶段开始函数
 * 从subtreeRoot开始遍历，判断subtreeFlags是否有标识
 * 如果有则将nextEffect指向子节点
 * 如果没有标识则调用commitLayoutMountEffects_complete
 * 
 * @param {*} subtreeRoot 
 * @param {*} root 
 * @param {*} committedLanes 
...
function commitLayoutEffects_begin(
  ...
  while (nextEffect !== null) {
    ...
    // 遍历fiber树，直到找到最下层有副作用的节点，也就是flag有LayoutMask标识的节点或者是叶子节点
...
/**
 * 横向遍历节点，判断节点flags上是否有LayoutMask标识
 * 如果有则调用commitLayoutEffectOnFiber函数
 * 然后将nextEffect指向兄弟节点
 * 如果不存在兄弟节点，则将nextEffect执行父节点
 * 
 * @param {*} subtreeRoot 
 * @param {*} root 
 * @param {*} committedLanes 
...
function commitLayoutMountEffects_complete(
  ...
  while (nextEffect !== null) {
    ...
    // 将nextEffect指向兄弟节点，并返回begin函数，如果兄弟节点存在flags，则在下一轮进入complete中，进入commitLayoutEffectOnFiber函数
    const sibling = fiber.sibling;
    if (sibling !== null) {
      sibling.return = fiber.return;
      nextEffect = sibling;
    ...
    // 如果没有兄弟节点，则将nextEffect指向父节点，进行向上遍历
    nextEffect = fiber.return;
  }
}
