...
export function reconcileChildren(
  ...
  if (current === null) {
    ...
    // 对于mount阶段，调用mountChildFibers方法，创建新的子Fiber节点
    ...
    // let's throw it out.
    // 对于update阶段，调用reconcileChildFibers方法，将当前组件与该组件在上次更新时创建的Fiber节点做比较，将比较结果生成新Fiber节点
...
function updateClassComponent(
  ...
  if (instance === null) {
    ...
    // 实例化类组件，并对updater进行初始化，
    constructClassInstance(workInProgress, Component, nextProps);
    // 进行类组件实例的一些初始化操作，如对props和state等的赋值，以及执行实例上的getDerivedStateFromProps、getSnapshotBeforeUpdate、componentWillMount方法
  ...
  // 执行实例的render方法，并通过调用reconcileChildren方法，创建子fiber节点并返回
...
function updateHostRoot(current, workInProgress, renderLanes) {
  ...
  // 状态计算，更新整合到workInProgress.memoizedState中来
  const nextProps = workInProgress.pendingProps;
  const prevState = workInProgress.memoizedState;
  const prevChildren = prevState.element;
  cloneUpdateQueue(current, workInProgress);
  // 遍历updateQueue.shared.pending，提取足够优先级的update对象，计算出最终的状态，存放在workInProgress.memoizedState中
  ...
  if (supportsHydration && prevState.isDehydrated) {
    ...
    // 根据ReactElement对象，调用reconcileChildren方法，生成Fiber子节点
    reconcileChildren(current, workInProgress, nextChildren, renderLanes);
  }
  return workInProgress.child;
}
...
function updateHostComponent(
  ...
  // hostComponent是原生标签组件，是无状态的，例如div、p等，所以只需要收集nextProps就可以
  ...
  if (isDirectTextChild) {
    ...
    // 如果是update状态，且之前子节点为文本节点，则将此次更新的fiber标识为内容重置
  ...
  markRef(current, workInProgress);
  // 根据ReactElement对象，调用reconcileChildren方法生成Fiber子节点
  reconcileChildren(current, workInProgress, nextChildren, renderLanes);
  return workInProgress.child;
}
...
function mountIndeterminateComponent(
  ...
  if (__DEV__) {
    ...
    // 执行组件函数，并获取执行后返回的值
    // 如果是函数组件，则返回vdom
    // 如果是类组件，则返回实例化后的组件对象，且包含render方法
    value = renderWithHooks(
      null,
      workInProgress,
      Component,
  ...
  if (
    ...
    // 在该组件实例上设置classComponentUpdater，并将fiber的stateNode执行该实例对象
    adoptClassInstance(workInProgress, value);
    // 进行类组件实例的一些初始化操作，如对props和state等的赋值，以及执行实例上的getDerivedStateFromProps、getSnapshotBeforeUpdate、componentWillMount方法
    mountClassInstance(workInProgress, Component, props, renderLanes);

    // 执行实例的render方法，并建立子fiber树
    return finishClassComponent(
      null,
      workInProgress,
      Component,
...
function beginWork(
  ...
  // 通过判断current判断是update阶段还是mount阶段
  if (current !== null) {
    // update阶段
  ...
  // 设置workInProgress的优先级，设为NoLanes(最高优先级)
  workInProgress.lanes = NoLanes;

  // 根据workInProgress的tag，进行不同的处理方法，创建子节点
  switch (workInProgress.tag) {
    // 不明确的组件，也就是不知道当前组件为什么类型
