...
export function markStarvedLanesAsExpired(
  ...
  while (lanes > 0) {
    ...
    // ~lane 逐位取反， &= 逐位比较，0返回false
    // 也就是将最前位置的1，置为0，遍历lanes所有的1，直到将lanes变为0
    lanes &= ~lane;
  }
}
...
/**
 * 从传入的车道中，选择优先度最高的车道，也就是最低位车道
 * @param {*} lanes 
 * @returns 
 */
export function getHighestPriorityLane(lanes: Lanes): Lane {
  // 将lanes取反后+1，并与lanes取并，得到最低位的车道
  // -lanes，需补位 = 取反后+1
  return lanes & -lanes;
}
...
/**
 * 返回转换为32位二进制后，有效数字个数，比如 参数16，转成32位二进制为00000000000000000000000000001000，返回 4
 * @param {*} lanes 
 * @returns 
 */
