...
/**
 * 1. 将需要更新的props以数组的形式存放在fiber的updateQueue上
 * 2. 如果存在需要更新的props，则将fiber.flags标记需要更新
 */
...
if (supportsMutation) {
  ...
  updateHostComponent = function(
    ...
    // related to `hidden`.
    // 处理props，返回需要更新处理后的props数组，奇数为prop的key值，偶数为value值
    // ex: [propKey1, propValue1, propsKey2, propsValue2...]
    ...
    // 将需要更新的props数组存放在fiber.updateQueue上，在commit阶段，再更改DOM节点，渲染到页面上
    workInProgress.updateQueue = (updatePayload: any);
    // If the update payload indicates that there is a change or if there
    // is a new ref we mark this as an update. All the work is done in commitWork.
    // 如果有需要更新的props，则标记该fiber为需要更新，也就是更改flags
    if (updatePayload) {
...
/**
 * 属性冒泡
 * 1. 将子孙fiber的lanes冒泡到当前fiber的lanes中
 * 2. 将子孙fiber的改动标记冒泡到当前fiber的subtreeFlags上
 * 3. 将所有子fiber的return指向自己
 * 4. 返回当前fiber是否复用
 * @param {*} completedWork 
 * @returns 
 */
function bubbleProperties(completedWork: Fiber) {
  // 如果alternate存在且alternate的子fiber与当前fiber的子相同，则是复用fiber
  const didBailout =
  ...
  if (!didBailout) {
    ...
    if (enableProfilerTimer && (completedWork.mode & ProfileMode) !== NoMode) {
      ...
      while (child !== null) {
        ...
        // 将child的return指向当前fiber
        child.return = completedWork;

        // 将child指向兄弟fiber，进行下一个fiber处理
        child = child.sibling;
      }
    }

    // 将subtree标记记录在fiber的subtreeFlags上，用于表示子孙fiber有哪些改动
  ...
  switch (workInProgress.tag) {
    ...
    case HostComponent: {
      ...
      if (current !== null && workInProgress.stateNode != null) {
        // update阶段且是复用原有fiber节点，已存在DOM节点
        updateHostComponent(
          current,
          workInProgress,
          type,
        ...
        if (current.ref !== workInProgress.ref) {
          // 如果当前fiber与上次更新的fiber的ref不同，则在flags标记ref更新
          markRef(workInProgress);
        }
      } else {
        // mount阶段 或 update阶段，没有复用fiber节点，新创建的fiber节点，也就是没有对应的DOM节点
        if (!newProps) {
        ...
        if (wasHydrated) {
          ...
          // 如果有子fiber，则将子fiber以及子fiber的兄弟fiber的stateNode，都挂载到当前DOM节点下
          appendAllChildren(instance, workInProgress, false, false);

          // 将当前fiber节点的stateNode指向DOM节点
          ...
          if (
            ...
            // 处理focus，如果props有autoFocus，则需要标记更新
            markUpdate(workInProgress);
          }
        }
