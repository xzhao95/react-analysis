...
/**
 * 重置render计时器，也就是重置workInProgressRootRenderTargetTime字段
 */
...
export function scheduleUpdateOnFiber(
  ...
  if (
    ...
    // 判断是否是新root（update过程中，需要替换的root分支）
...
function ensureRootIsScheduled(root: FiberRoot, currentTime: number) {
  ...
  // 检查是否有即将超时的任务，如果有，则标记他们超时，优先执行
  markStarvedLanesAsExpired(root, currentTime);

  // Determine the next lanes to work on, and their priority.
  // 安排下一个执行的任务和他们的优先级
  // mount阶段，nextLanes为16
  // 获取当前任务的优先级
  ...
  // 判断是否有任务，没有的话，重置callbackNode，并且退出
  ...
  // 使用最高优先级lane去代表callback的优先级
  const newCallbackPriority = getHighestPriorityLane(nextLanes);

  // Check if there's an existing task. We may be able to reuse it.
  // 检查是否有正在执行的任务
  const existingCallbackPriority = root.callbackPriority;
  // 比对一下两者的优先级，如果优先级一样，则正在执行的任务优先级也很高，直接返回null，让当前正在执行的任务继续执行
  if (
  ...
  // 否则，当前的任务优先级更高，暂停正在执行的任务，优先调度当前任务
  if (existingCallbackNode != null) {
  ...
  // 在scheduleCallback中创建一个新任务，然后加入到任务队列中
  let newCallbackNode;
  // 根据当前的优先级，判断是同步执行，还是异步执行
  if (newCallbackPriority === SyncLane) {
    // 同步执行performSyncWorkOnRoot方法
    ...
    // 异步执行performConcurrentWorkOnRoot方法
    // 在concurrent模式下，注册任务，并加入到任务队列中
  ...
  // 更高优先级的任务，会被赋值到root.callbackNode上
...
function performConcurrentWorkOnRoot(root, didTimeout) {
  ...
  if (exitStatus !== RootInProgress) {
    ...
    if (exitStatus === RootDidNotComplete) {
      ...
      // 渲染fiber树，也就是commit阶段的起始，对应commitRoot
      finishConcurrentRender(root, exitStatus, lanes);
    }
  }

  // 退出前，检查是否还有其他更新，是否需要发起新调度
  ensureRootIsScheduled(root, now());
  if (root.callbackNode === originalCallbackNode) {
    // The task node scheduled for this root is the same one that's
    // currently executed. Need to return a continuation.
    // 渲染被阻断，返回一个新的performConcurrentWorkOnRoot等待下一次被调用
...
function renderRootConcurrent(root: FiberRoot, lanes: Lanes) {
  ...
  if (workInProgressRoot !== root || workInProgressRootRenderLanes !== lanes) {
    ...
    workInProgressTransitions = getTransitionsForLanes(root, lanes);
    resetRenderTimer();
    // 用于创建workInProgress Fiber的rootFiber
    // 通过RootFiber.alternate与workInProgress Fiber关联，workInProgress Fiber.alternate也指向RootFiber
    prepareFreshStack(root, lanes);
  }
...
function workLoopConcurrent() {
  ...
  // 跟同步执行差别在于shouldYield，如果浏览器帧剩余时间不足，则暂停任务，等浏览器有空闲时间再重新执行
...
function performUnitOfWork(unitOfWork: Fiber): void {
  ...
  // 归阶段，也就是冒泡阶段，主要使用completeUnitOfWork方法
  // 如果没有有子fiber则执行completeUnitOfWork，如果有，则将子fiber作为正在处理的fiber，也就是workInProgress
  if (next === null) {
...
function completeUnitOfWork(unitOfWork: Fiber): void {
  ...
  do {
    ...
    // nothing should rely on this, but relying on it here means that we don't
    // need an additional field on the work in progress.
    // 当前fiber，也就是上次更新创建的fiber
    const current = completedWork.alternate;
    // 父fiber
    const returnFiber = completedWork.return;

    // Check if the work completed or if something threw.
    // 检查当前fiber是否已经标记已经完成处理
    ...
    if (siblingFiber !== null) {
      // 如果当前fiber有兄弟节点，则将兄弟节点作为当前执行fiber，返回执行beginWork
      // If there is more work to do in this returnFiber, do that next.
      workInProgress = siblingFiber;
      return;
    }
    // Otherwise, return to the parent
    // 否则将父fiber作为completedWork，继续对父fiber做completeWork处理
...
function commitRootImpl(
  ...
  do {
    ...
    // 触发useEffect回调与其他同步任务。由于这些任务可能触发其他渲染，所以一直循环直到没有任务
    flushPassiveEffects();
  } while (rootWithPendingPassiveEffects !== null); // mount阶段为null
  ...
  if (subtreeHasEffects || rootHasEffect) {
    ...
    // 切换fiber树，将root.current指向当前更新创建的fiber树
    ...
    // No effects.
    // 没有副作用，没有更新，直接将root.current指向此次更新创建的RootFiber
    root.current = finishedWork;
...
export function flushPassiveEffects(): boolean {
  ...
  // 会从rootWithPendingPassiveEffects中获取effectsList
