...
export function updateContainer(
  ...
  // 初次渲染：fiberRoot指向的rootFiberNode
  const current = container.current;
  const eventTime = requestEventTime();
  // 获取优先级lane
  // lane模型，用二进制表示优先级，1的bits越多，优先级越低， 目前lane为defaultLane = 32， 0b0000000000000000000000000100000;
  const lane = requestUpdateLane(current);
  ...
  // 此时parentComponent为null，所以context为{}
  ...
  // 将<App/>存放在payload中，此时<App/>以被babel解析为React.createElement(App)，并执行返回
  ...
  // update入列处理
  // 配置update链表，并将queue插入进concurrentQueues队列中，返回的root是FiberNodeRoot
